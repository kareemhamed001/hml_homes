...
class OffPlan {
    ...
    async listCustomers(project_id,page=1, perPage = 8,showLoader = true) {
        try {
            let request = new RequestBuilder()
                .setUrl(routes.dashboard.project.customers.url.replace("{projects}", project_id) + "?page=" + page + "&pagination=" + perPage)
                .setMethod(routes.dashboard.project.customers.method)
                .setHeaders({'Accept': 'application/json', ...this._headers})
                .setResponseType(this._responseType)
                .setPreviewType(this._previewType)
                .setShowLoader(showLoader)
